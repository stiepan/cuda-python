...
cdef extern from "include/layout.hpp":
    ...
    void _order_from_strides(axis_order_t& indices, extent_t* extent_t, stride_t* stride_t, int ndim) except + nogil
...
cdef struct BaseLayout:
    ...
    # Use ``_resize_layout`` to initialize the layout, it will
...
cdef class StridedLayout:
    ...
    cdef inline stride_t _init_dense(StridedLayout self, BaseLayout& base, int itemsize, OrderFlag order_flag, axis_order_t* stride_order=NULL) except -1 nogil:
        _validate_itemsize(itemsize)
    ...
    cdef inline int init_dense_from_ptr(StridedLayout self, int ndim, extent_t* shape, int itemsize, OrderFlag order_flag, axis_order_t* stride_order=NULL) except -1 nogil:
        cdef BaseLayout base
    ...
    cdef inline int get_stride_order(StridedLayout self, axis_order_t& stride_order) except -1 nogil:
    ...
    cdef inline bint get_is_contiguous_c(StridedLayout self) except -1 nogil:
        ...
        return _set_boolean_property(self, PROP_IS_CONTIGUOUS_C, _is_contiguous_c(self.base))
    ...
    cdef inline bint get_is_contiguous_f(StridedLayout self) except -1 nogil:
        ...
        return _set_boolean_property(self, PROP_IS_CONTIGUOUS_F, _is_contiguous_f(self.base))
    ...
    cdef inline bint get_is_contiguous_any(StridedLayout self) except -1 nogil:
        ...
        cdef axis_order_t stride_order
        self.get_stride_order(stride_order)
        return _set_boolean_property(self, PROP_IS_CONTIGUOUS_ANY, _is_contiguous_any(self.base, stride_order))
...
cdef inline int _resize_layout(BaseLayout& layout, int ndim) except -1 nogil:
...
cdef inline void _assure_strides_ptr(BaseLayout& base) noexcept nogil:
    if base.strides == NULL:
        base.strides = base._mem.data() + base.ndim
...
cdef inline void _zeros_strides(BaseLayout& base) noexcept nogil:
...
cdef inline stride_t _dense_strides_in_order(BaseLayout& base, axis_order_t& stride_order) except? -1 nogil:
    cdef int ndim = base.ndim
...
cdef inline bint _is_contiguous_c(BaseLayout& base) except -1 nogil:
    if base.strides == NULL:
        return True
...
cdef inline bint _is_contiguous_f(BaseLayout& base) except -1 nogil:
    if base.strides == NULL:
        return False
    cdef int ndim = base.ndim
    cdef int64_t stride = 1
...
cdef inline bint _is_contiguous_any(BaseLayout& base, axis_order_t& axis_order) except -1 nogil:
    if base.strides == NULL:
        return True
    cdef int64_t stride = 1
    cdef int64_t j = base.ndim - 1
...
cdef inline OrderFlag _stride_order2vec(axis_order_t& stride_order_vec, object stride_order) except? ORDER_NONE:
    if stride_order == 'C':
        return ORDER_C
    elif stride_order == 'F':
        return ORDER_F
    elif isinstance(stride_order, tuple | list): 
        _tuple2axis_order(stride_order_vec, stride_order)
        return ORDER_PERM
    return ORDER_NONE


cdef inline int _tuple2axis_order(axis_order_t& vec, object t) except -1:
    cdef int ndim = len(t)
    vec.resize(ndim)
    for i in range(ndim):
        vec[i] = t[i]
...
cdef inline bint _is_unique(BaseLayout& base, axis_order_t& stride_order) except -1 nogil:
    if base.strides == NULL:
        return True
